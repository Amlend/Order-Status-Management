...
const executeOrder = () => {
	...
	chefReceived(orderIdInput)
		...
		.then(stirring)
		.then(addingCream)
		.then(garnisingCilantro)
		.then(chickenCooked)
		.then(cultryaddedandpacked)
		.then(packageReceivedAtCounter)
		.then(() => document.getElementById(orderIdInput).innerText = 'Order Ready!')
		.then(()=>document.getElementById("final").innerHTML='<img src=images/chicken.gif>')
...
const fryingChicken = (orderNumber) => {
	return new Promise((resolve, reject) => {
		setTimeout(() => {
			document.getElementById(orderNumber).innerText = 'Frying Chicken on both sides'
			resolve(orderNumber)
		}, 5000)
	})
}

const fryingOnion = (orderNumber) => {
	return new Promise((resolve, reject) => {
		setTimeout(() => {
			document.getElementById(orderNumber).innerText = 'Frying Onion, Garlic and Ginger'
			resolve(orderNumber)
		}, 3000)
	})
}

const addingTomatoes = (orderNumber) => {
	return new Promise((resolve, reject) => {
		setTimeout(() => {
			document.getElementById(orderNumber).innerText = 'Adding crushed tomatoes, chili powder'
			resolve(orderNumber)
		}, 3000)
	})
}
const stirring = (orderNumber) => {
	return new Promise((resolve, reject) => {
		setTimeout(() => {
			document.getElementById(orderNumber).innerText = 'Stirring occasionally until sauce thickens'
			resolve(orderNumber)
		}, 4000)
	})
}

const addingCream = (orderNumber) => {
	return new Promise((resolve, reject) => {
		setTimeout(() => {
			document.getElementById(orderNumber).innerText = 'Adding cream, sugar & kasoori methi'
			resolve(orderNumber)
		}, 5000)
	})
}

const garnisingCilantro = (orderNumber) => {
	return new Promise((resolve, reject) => {
		setTimeout(() => {
			document.getElementById(orderNumber).innerText = 'Garnishing with chopped cilantro'
			resolve(orderNumber)
		}, 3000)
	})
}

const chickenCooked = (orderNumber) => {
	return new Promise((resolve, reject) => {
		setTimeout(() => {
			document.getElementById(orderNumber).innerText = 'Butter Chicken Cooked🍗'
			resolve(orderNumber)
		}, 10000)
	})
}
...
const createNewOrderColumn = (orderIdInput) => {
	...
	let cardTitleH5Text = document.createTextNode('Bill Amount: ₹580')
	let cardTextPText = document.createTextNode('Butter Chicken Order Placed')
	let cardButtonAText = document.createTextNode('Cancel Order')
	let cardOrderStatusText = document.createTextNode('Butter Chicken is on the way')
	...
	colDiv.classList = 'col-md-4 col-xs-12 text-center order-block'
